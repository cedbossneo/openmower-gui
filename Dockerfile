...
RUN git clone https://github.com/raspberrypi/openocd.git --recursive --depth=1 && cd openocd && ./bootstrap && ./configure --enable-ftdi --enable-sysfsgpio --enable-bcm2835gpio && make -j$(nproc) && make install && cd .. && rm -rf openocd
