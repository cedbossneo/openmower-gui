...
func (fp *FirmwareProvider) flashVermut(writer io.Writer, config FirmwareConfig) error {
	...
	cmd = execabs.Command("/bin/bash", "-c", "openocd -f interface/raspberrypi-swd.cfg -f target/rp2040.cfg -c \"program "+os.TempDir()+"/firmware/firmware/"+config.Version+"/firmware.elf verify reset exit\"")
	cmd.Stdout = writer
	cmd.Stderr = writer
	err = cmd.Run()
	if err != nil {
